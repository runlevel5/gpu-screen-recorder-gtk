                           ...
                           void *user_data)
{
    ...
    /* Wrap the page content in a titled XmFrame ("Recording"). The frame
     * itself owns the form attachments; its inner RowColumn (returned by
     * gsr_w_titled_frame) is what page widgets get parented under. */
    Widget frame = NULL;
    Widget rc = gsr_w_titled_frame(out->root, "Recording", &frame);
    XtVaSetValues(frame,
        XmNtopAttachment,    XmATTACH_FORM,
        XmNleftAttachment,   XmATTACH_FORM,
        XmNrightAttachment,  XmATTACH_FORM,
